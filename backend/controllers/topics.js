...
async function updateTopic(req, res) {
  try {
    const topic = await Topic.findOne({ where: { id: req.body.topicID } }).then(
      (topic) => {
        return topic;
      }
    );
...
async function deleteTopic(req, res) {
  try {
    const topic = await Topic.findOne({ where: { id: req.body.id } }).then(
      (topic) => {
        return topic;
      }
    );
...
async function getMessagesTopic(req, res) {
  try {
    ...
    if (msg.length == 0) {
      res.status(200).send("Aucun message n'a été trouvé");
    } else {
